...
import dataStructures.database.honors.HonorsProject;
import dataStructures.database.honors.ProjectRepository;

@Controller
@RequestMapping("projectapi/Projects")
public class ProjectAPIControllerProject {
	...
	public ResponseEntity<String> getAllProjects() {
		try {
			...
			for(HonorsProject current : dbProjects.findAll()) {
	...
	public ResponseEntity<String> createProject(@RequestHeader(value="apiKey") String key, @RequestBody String bodyString) {
		try {
			...
			if(validateKey(key)) {
				HonorsProject newProject = new HonorsProject(body);
