...
import dataStructures.database.honors.ghosty.aspect.HonorsAspect;
import dataStructures.database.honors.ghosty.aspect.AspectRepository;
import dataStructures.database.honors.ghosty.ghost.HonorsGhost;
import dataStructures.database.honors.ghosty.ghost.GhostRepository;
import services.WebRequester;

@Controller
@RequestMapping("projectapi/ghosty/")
public class GhostyAPIControllerProject {
	...
	public ResponseEntity<String> getAllGhosts() {
		try {
			...
			for(HonorsGhost current : ghosts.findAll()) {
	...
	public ResponseEntity<String> getAllAspects() {
		try {
			...
			for(HonorsAspect current : aspects.findAll()) {
		...
		try {
			...
			if(validateKey(key)) {
				if(body.has("evidence") && body.has("name") && body.has("description")) {
					HonorsGhost newGhost = new HonorsGhost(body);
		...
		try {
			...
			if(validateKey(key)) {
				if(body.has("name") && body.has("content") && body.has("type")) {
					HonorsAspect newAspect = new HonorsAspect(body);
